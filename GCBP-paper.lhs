...
\todo{Need pictures and/or more intuitive discussion before
  ...
  doing.} To express this formally, we introduce the function |dom|
(short for \emph{domain}), which intuitively extracts just the
``effects'' of a morphism |f|: for each |a|, the effects generated by
running |f a| are retained, but the output of type |b| is discarded
and replaced by |a| itself.  This is illustrated in \pref{fig:dom},
and an implementation of |dom| is shown in \pref{fig:domcode}.
\begin{figure}[htp]
  \centering
...
the definition of |(<||||>)| for partial bijections; we will see the
...
\item and similarly define $U'$, $V'$, and $W'$ in terms of $\beta$,
  $B^+$, and $B^-$.
\item Define $v$ by the action of $\alpha$ on $V$---
\end{itemize}
